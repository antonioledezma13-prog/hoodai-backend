...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).json({ token: sign(user._id), user: { id: user._id, name, email, lang, role: user.role } });
  } catch (e) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ token: sign(user._id), user: { id: user._id, name: user.name, email, lang: user.lang, role: user.role } });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
